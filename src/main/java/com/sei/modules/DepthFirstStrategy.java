...
public class DepthFirstStrategy {
    ...
    public Decision make(int id, ViewTree currentTree, ViewTree newTree, Decision prev_decision, int response){
        ...
        if (prev_decision.code == Decision.CODE.CONTINUE && response != Device.UI.SAME
            ...
            update_graph(device, prev_decision, currentTree, newTree);
    ...
    public void update_graph(Device d, Decision prev_decision, ViewTree currentTree, ViewTree newTree){
        graphAdjustor.update(d, prev_decision.action, currentTree, newTree);
    }
}
