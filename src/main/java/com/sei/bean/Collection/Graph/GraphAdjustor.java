...
public class GraphAdjustor extends UiTransition{
    ...
    public int update(Device d, Action action, ViewTree currentTree, ViewTree new_tree){
        int id = d.id;
        if (action == null){
        ...
        if (!REPLAY_MODE) {
            Handler handler = handler_table.get(queryGraph(d, currentTree, new_tree));
    ...
    public int queryGraph(Device d, ViewTree currentTree, ViewTree new_tree){
        int id = d.id;
